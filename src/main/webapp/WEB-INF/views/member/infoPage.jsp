...
//제이쿼리 : $(function(){ ... });  jQuery(documnet).ready(function(){ ... }); 과 같음
...
<%-- 뷰 페이지에서는 안보이면서, 서버로 전송하는 폼 안에 같이 전송시킬 값을 포함시키는 경우 input type="hidden" 사용함 --%>
	...
	<tr><th>사진첨부</th>
			...
			<c:if test="${ !empty requestScope.member.photoFileName }">
				<div id="myphoto" style="margin:0; width:150px; height:160px; padding:0; border:1px solid navy;">
					...
					<img alt="사진을 드래그 드롭하세요." src="/first/resources/photoFiles/${ requestScope.member.photoFileName }" id="photo"
					style="width:150px;height:160px;border:1px solid navy;display:block;margin:0;padding:0;">
				</div><br>
				${ requestScope.ofile } <br>
				변경한 사진 선택 : <input type="file" id="photofile" name="photofile">
			 </c:if>
			 <c:if test="${ empty requestScope.member.photoFileName }">
				<div id="myphoto" style="margin:0; width:150px; height:160px; padding:0; border:1px solid navy;">
					<%-- 사진 첨부가 없을 경우를 위한 미리보기용 이미지 출력되게 함 --%>
					<img alt="사진을 드래그 드롭하세요." src="/first/resources/images/photo1.jpg" id="photo"
				style="width:150px;height:160px;border:1px solid navy;display:block;margin:0;padding:0;">
				</div><br>
